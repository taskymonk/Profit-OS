...
export function calculateDashboardMetrics(orders, skuRecipes, expenses, startDate, endDate, exchangeRate = 1, adSpendMap = {}, adSpendTaxMultiplier = 1) {
  ...
  // Strict Accounting Parity: exclude Cancelled, Voided, and Pending orders from
  // financial metrics. They appear in the raw orders table but NOT in revenue/profit calcs.
  const EXCLUDED_STATUSES = ['Cancelled', 'Voided', 'Pending'];
  const accountingOrders = filteredOrders.filter(o => !EXCLUDED_STATUSES.includes(o.status));
