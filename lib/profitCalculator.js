...
export function calculateDashboardMetrics(orders, skuRecipes, expenses, startDate, endDate, exchangeRate = 1, adSpendMap = {}) {
  ...
  // Pre-compute orders per day for efficient lookup
  const ordersPerDay = {};
  ...
  // Calculate profit for each filtered order using adSpendMap
  const orderProfits = filteredOrders.map(order => {
    ...
    return calculateOrderProfit(order, skuMap[order.sku], dayAdSpend, dayOrderCount, exchangeRate);
  });
  ...
  // Sum ad spend only for days within the filtered range
  let totalAdSpend = 0;
  ...
  // Pro-rate recurring monthly overhead for this date range (excludes MetaAds)
