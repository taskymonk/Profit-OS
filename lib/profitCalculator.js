...
export function calculateOrderProfit(order, skuRecipe, dailyAdSpend = 0, totalDailyOrders = 1, exchangeRate = 1, adSpendTaxMultiplier = 1) {
  ...
  // 4. Transaction Fees — Razorpay reconciled > COD zero > predictive 2%+GST
  if (order.razorpayReconciled === true) {
    ...
    // COD orders: gateway fee = ₹0 (shipping tariffs handle COD charges)
    result.gatewayFee = 0;
    result.gstOnGateway = 0;
  } else {
    // Predictive fallback: 2% + 18% GST on fee
