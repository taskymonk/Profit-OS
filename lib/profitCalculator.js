...
export function calculateOrderProfit(order, skuRecipe, dailyAdSpend = 0, totalDailyOrders = 1, exchangeRate = 1, adSpendTaxMultiplier = 1, options = {}) {
  const consumptions = options.consumptions || []; // FIFO consumption records for this order
  const result = {
    orderId: order.id || order._id,
    sku: order.sku,
    status: order.status,
    // Shopify Parity: grossRevenue = salePrice MINUS tip (Shopify Total Sales excludes tips)
    grossRevenue: (order.salePrice || 0) - (order.tipAmount || 0),
  ...
  const orderQty = order.quantity || 1; // Multi-quantity orders multiply COGS
  ...
  // Shopify Parity: netRevenue = revenue after deductions minus tax (tip already excluded from grossRevenue)
  result.netRevenue = revenueAfterDeductions - result.gstOnRevenue;

  // 2. COGS from SKU Recipe — FIFO Consumption (priority) > BOM Architecture > Legacy fallback
  if (consumptions.length > 0) {
    // FIFO: Use actual batch costs from consumption records
    consumptions.forEach(c => {
      const cat = (c.category || c.inventoryItemCategory || '').toLowerCase();
      if (cat.includes('packag')) result.packagingCost += (c.totalCost || 0);
      else if (cat.includes('raw') || cat === 'raw material') result.rawMaterialCost += (c.totalCost || 0);
      else result.consumableCost += (c.totalCost || 0);
    });
    // Any unclassified consumption goes to raw material
    const classifiedTotal = result.rawMaterialCost + result.packagingCost + result.consumableCost;
    const consumptionTotal = consumptions.reduce((s, c) => s + (c.totalCost || 0), 0);
    if (consumptionTotal > classifiedTotal) result.rawMaterialCost += (consumptionTotal - classifiedTotal);
    const subtotalCOGS = consumptionTotal;
    const wastagePercent = skuRecipe ? (skuRecipe.monthlyWastageOverride || skuRecipe.defaultWastageBuffer || 0) : 0;
    ...
    if (skuRecipe.ingredients && skuRecipe.ingredients.length > 0) {
      // BOM: each ingredient has baseCostPerUnit and quantityUsed (per unit)
      // Multiply by orderQty for multi-quantity orders
      (skuRecipe.ingredients).forEach(ing => {
        const baseCost = ing.baseCostPerUnit || 0;
        const qtyUsed = ing.quantityUsed || ing.quantity || 0;
        const lineCost = qtyUsed * baseCost * orderQty;
      ...
      // Legacy format: rawMaterials + packaging + consumableCost — multiply by orderQty
      ...
      result.consumableCost = (skuRecipe.consumableCost || 0) * orderQty;
      const subtotalCOGS = result.rawMaterialCost + result.packagingCost + result.consumableCost;
      const wastagePercent = skuRecipe.monthlyWastageOverride || skuRecipe.defaultWastageBuffer || 0;
      result.wastageCost = subtotalCOGS * (wastagePercent / 100);
      result.totalCOGS = subtotalCOGS + result.wastageCost;
...
export function calculateProratedOverhead(expenses, startDate, endDate) {
  ...
  return {
    monthlyTotal: uniqueExpenses.filter(e => {
      const f = e.frequency || 'monthly';
      return f === 'monthly' || f === 'recurring';
...
export function calculateDashboardMetrics(orders, skuRecipes, expenses, startDate, endDate, exchangeRate = 1, adSpendMap = {}, adSpendTaxMultiplier = 1, options = {}) {
  ...
  const consumptionMap = options.consumptionMap || {}; // { orderId: [consumption records] }
  const orderProfits = filteredOrders.map(order => {
    ...
    const orderOpts = { ...options, consumptions: consumptionMap[order._id || order.id] || [] };
    return calculateOrderProfit(order, skuMap[order.sku], dayAdSpend, dayOrderCount, exchangeRate, adSpendTaxMultiplier, orderOpts);
  });
  ...
  const grossOrderProfits = accountingOrders.map(order => {
    const orderOpts = { ...options, consumptions: consumptionMap[order._id || order.id] || [] };
    return calculateOrderProfit(order, skuMap[order.sku], 0, 1, exchangeRate, 1, orderOpts);
  });

  const grossOrderProfit = grossOrderProfits.reduce((sum, p) => sum + p.netProfit, 0);
  // Count unique Shopify orders (not rows/line-items) for Shopify Analytics parity
  const uniqueOrderIds = new Set(accountingOrders.map(o => o.shopifyOrderId || o._id || o.id));
  const totalOrders = uniqueOrderIds.size;
  const uniqueRtoIds = new Set(accountingOrders.filter(o => o.status === 'RTO').map(o => o.shopifyOrderId || o._id || o.id));
  const rtoOrders = uniqueRtoIds.size;
  const rtoRate = totalOrders > 0 ? (rtoOrders / totalOrders) * 100 : 0;
  // Revenue uses grossRevenue (which already excludes tips) for Shopify parity
  const totalRevenue = accountingOrders.reduce((sum, o) => sum + ((o.salePrice || 0) - (o.tipAmount || 0)), 0);
  const uniqueCancelledIds = new Set(
    filteredOrders.filter(o =>
      EXCLUDED_STATUSES.includes(o.status) || EXCLUDED_FINANCIAL.includes(o.financialStatus)
    ).map(o => o.shopifyOrderId || o._id || o.id)
  );
  const cancelledCount = uniqueCancelledIds.size;

  let totalAdSpend = 0;
  let totalAdSpendRaw = 0;
  for (const [date, amount] of Object.entries(adSpendMap)) {
    const d = new Date(date + 'T00:00:00+05:30');
    if (d >= start && d <= end) {
      totalAdSpendRaw += amount;
  ...
  const totalTips = accountingOrders.reduce((s, o) => s + (o.tipAmount || 0), 0);

  // trueNetProfit: grossOrderProfit already includes Razorpay + Shopify txn fees
  // (calculated with dayAdSpend=0 to isolate order-level costs, ad spend added globally below)
  // Only subtract: ad spend (global), overhead (global). Shopify fee is already in grossOrderProfit.
  ...
  return {
    ...
    plBreakdown: {
      ...
      shopifyTxnFee: r2(totalShopifyTxnFee),
      shopifyTxnGST: r2(totalShopifyTxnGST),
      totalShopifyFee: r2(totalShopifyFee),
      adSpend: r2(totalAdSpend),
      adSpendRaw: r2(totalAdSpendRaw),
      adSpendTax: r2(totalAdSpend - totalAdSpendRaw),
      overhead: r2(overhead.proratedAmount),
      overheadCategoryBreakdown: overhead.categoryBreakdown,
      netProfit: r2(trueNetProfit),
    },
    orderProfits,
  };
