...
export const CARRIERS = {
  ...
  ekart: { name: 'Ekart Logistics', shortName: 'Ekart', color: '#2874F0',
    trackUrl: (t) => `https://www.ekartlogistics.com/track/${t}`,
    patterns: [/^FMPC\d{10,14}$/i, /^[A-Z]{3,5}\d{10,14}$/],
    builtIn: true,
  },
  amazonswa: { name: 'Amazon Shipping', shortName: 'Amazon', color: '#FF9900',
    trackUrl: (t) => `https://track.amazon.in/tracking/${t}`,
    patterns: [/^[A-Z0-9]{12,20}$/],
    builtIn: true,
  },
...
// Carrier keywords — expanded for better label detection
const CARRIER_KEYWORDS = {
  indiapost: INDIA_POST_KEYWORDS,
  bluedart: ['blue dart', 'bluedart', 'blue dart express', 'blue dart aviation', 'bluedart.com'],
  delhivery: ['delhivery', 'delhivry', 'delhivery.com', 'delhivery logistics'],
  dtdc: ['dtdc', 'dtdc express', 'dtdc courier', 'dtdc.in'],
  xpressbees: ['xpressbees', 'xpress bees', 'xpressbees.com'],
  fedex: ['fedex', 'federal express', 'fedex express', 'fedex.com'],
  ecom: ['ecom express', 'ecomexpress', 'ecom.in'],
  shadowfax: ['shadowfax', 'shadowfax technologies'],
  ekart: ['ekart', 'ekart logistics', 'flipkart logistics'],
  amazonswa: ['amazon shipping', 'amazon logistics', 'swa', 'amzl'],
};
...
export function extractTrackingInfo(ocrText) {
  ...
  // === STEP 2: Carrier-specific FAST PATHs ===
  // When a carrier is detected from keywords, use targeted regex for that carrier's format

  // Helper: clean OCR spaces from a number candidate
  const cleanSpaces = (str) => str.replace(/\s+/g, '');

  // --- India Post S10 Fast Path ---
  if (detectedCarrier === 'indiapost' || textLower.includes('speed post') || textLower.includes('india post') || textLower.includes('ems')) {
    // S10 format: XX123456789IN or XX123456789XX (with possible OCR spaces)
    const s10Regex = /([A-Z]{2}\s*\d[\d\s]{7,10}\d\s*[A-Z]{2})/gi;
    let match;
    const s10Candidates = [];
    while ((match = s10Regex.exec(text)) !== null) {
      const cleaned = cleanSpaces(match[1]).toUpperCase();
      if (/^[A-Z]{2}\d{9}[A-Z]{2}$/.test(cleaned)) {
        s10Candidates.push(cleaned);
      }
    }

    const prefixMatches = s10Candidates.filter(c => INDIA_POST_PREFIXES.includes(c.substring(0, 2)));
  ...
  // --- Blue Dart Fast Path ---
  if (detectedCarrier === 'bluedart') {
    // BlueDart: letter + 10 digits OR 11 digits, often labeled "AWB No"
    const bdPatterns = [
      /(?:awb|waybill|tracking|docket)[\s.:=#\-]*([A-Z]?\d{10,11})/gi,
      /\b([A-Z]\d{10})\b/g,
      /\b(\d{11})\b/g,
    ];
    for (const pat of bdPatterns) {
      ...
      while ((m = pat.exec(text)) !== null) {
        const cleaned = cleanSpaces(m[1]);
        if (cleaned.length >= 10 && cleaned.length <= 12 && !isIndianPhoneNumber(cleaned)) {
          return { trackingNumber: cleaned, carrier: 'bluedart', confidence: 0.95, allCandidates: [cleaned], rawText: text.substring(0, 500) };
        }
      }
    }
  }
  ...
  // --- Delhivery Fast Path ---
  if (detectedCarrier === 'delhivery') {
    // Delhivery: 13-18 digit numeric waybill
    const delPatterns = [
      /(?:awb|waybill|tracking|wb)[\s.:=#\-]*(\d[\d\s]{11,19})/gi,
      /\b(\d{13,18})\b/g,
    ];
    for (const pat of delPatterns) {
      ...
      while ((m = pat.exec(text)) !== null) {
        const cleaned = cleanSpaces(m[1]);
        if (cleaned.length >= 13 && cleaned.length <= 18 && /^\d+$/.test(cleaned)) {
          return { trackingNumber: cleaned, carrier: 'delhivery', confidence: 0.95, allCandidates: [cleaned], rawText: text.substring(0, 500) };
        }
      }
    }
  }
  ...
  // --- DTDC Fast Path ---
  if (detectedCarrier === 'dtdc') {
    // DTDC: D+8-9 digits, or XX+7-9 digits, or labeled tracking
    const dtdcPatterns = [
      /(?:awb|cn|consignment|tracking|ref)[\s.:=#\-]*([A-Z0-9]{8,12})/gi,
      /\b([A-Z]\d{8,9})\b/g,
      /\b([A-Z]{2}\d{7,9})\b/g,
      /\b(D\d{8,9})\b/gi,
    ];
    for (const pat of dtdcPatterns) {
      pat.lastIndex = 0;
      let m;
      while ((m = pat.exec(text)) !== null) {
        const cleaned = cleanSpaces(m[1]);
        if (cleaned.length >= 8 && cleaned.length <= 12 && !isIndianPhoneNumber(cleaned)) {
          return { trackingNumber: cleaned.toUpperCase(), carrier: 'dtdc', confidence: 0.93, allCandidates: [cleaned], rawText: text.substring(0, 500) };
        }
      }
    }
  }

  // --- XpressBees Fast Path ---
  if (detectedCarrier === 'xpressbees') {
    // XpressBees: 14-15 digit numeric
    const xbPatterns = [
      /(?:awb|waybill|tracking)[\s.:=#\-]*(\d[\d\s]{12,16})/gi,
      /\b(\d{14,15})\b/g,
    ];
    for (const pat of xbPatterns) {
      pat.lastIndex = 0;
      let m;
      while ((m = pat.exec(text)) !== null) {
        const cleaned = cleanSpaces(m[1]);
        if (cleaned.length >= 14 && cleaned.length <= 15 && /^\d+$/.test(cleaned)) {
          return { trackingNumber: cleaned, carrier: 'xpressbees', confidence: 0.94, allCandidates: [cleaned], rawText: text.substring(0, 500) };
        }
      }
    }
  }
  ...
  if (detectedCarrier === 'fedex') {
    ...
    const fxPatterns = [
      /(?:tracking|awb|waybill)[\s.:=#\-]*(\d[\d\s]{10,23})/gi,
      /\b(\d{12})\b/g,
      /\b(\d{15})\b/g,
      /\b(\d{20,22})\b/g,
    ];
    for (const pat of fxPatterns) {
      pat.lastIndex = 0;
      let m;
      while ((m = pat.exec(text)) !== null) {
        const cleaned = cleanSpaces(m[1]);
        if ([12, 15, 20, 21, 22].includes(cleaned.length) && /^\d+$/.test(cleaned)) {
          return { trackingNumber: cleaned, carrier: 'fedex', confidence: 0.94, allCandidates: [cleaned], rawText: text.substring(0, 500) };
        }
      }
    }
  }

  // --- Ecom Express Fast Path ---
  if (detectedCarrier === 'ecom') {
    const ecPatterns = [
      /(?:awb|waybill|tracking)[\s.:=#\-]*(\d[\d\s]{8,13})/gi,
      /\b(\d{10,12})\b/g,
    ];
    for (const pat of ecPatterns) {
      pat.lastIndex = 0;
      let m;
      while ((m = pat.exec(text)) !== null) {
        const cleaned = cleanSpaces(m[1]);
        if (cleaned.length >= 10 && cleaned.length <= 12 && /^\d+$/.test(cleaned) && !isIndianPhoneNumber(cleaned)) {
          return { trackingNumber: cleaned, carrier: 'ecom', confidence: 0.93, allCandidates: [cleaned], rawText: text.substring(0, 500) };
        }
      }
    }
  }
  ...
  // --- Shadowfax Fast Path ---
  if (detectedCarrier === 'shadowfax') {
    const sfRegex = /\b(SF[A-Z0-9]{10,14})\b/gi;
    let m;
    while ((m = sfRegex.exec(text)) !== null) {
      return { trackingNumber: m[1].toUpperCase(), carrier: 'shadowfax', confidence: 0.95, allCandidates: [m[1]], rawText: text.substring(0, 500) };
    }
  }

  // --- Ekart / Amazon Fast Path ---
  if (detectedCarrier === 'ekart' || detectedCarrier === 'amazonswa') {
    const ekPatterns = [
      /(?:awb|tracking|shipment)[\s.:=#\-]*([A-Z0-9]{10,20})/gi,
      /\b(FMPC\d{10,14})\b/gi,
      /\b([A-Z]{3,5}\d{10,14})\b/g,
    ];
    for (const pat of ekPatterns) {
      pat.lastIndex = 0;
      let m;
      while ((m = pat.exec(text)) !== null) {
        const cleaned = cleanSpaces(m[1]);
        if (cleaned.length >= 10 && !isIndianPhoneNumber(cleaned)) {
          return { trackingNumber: cleaned.toUpperCase(), carrier: detectedCarrier, confidence: 0.9, allCandidates: [cleaned], rawText: text.substring(0, 500) };
        }
      }
    }
  }
